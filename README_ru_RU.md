...
Основано на [Phillip M. Duxbury's C++ Cheatsheet](http://www.pa.msu.edu/~duxbury/courses/phy480/Cpp_refcard.pdf), английская версия доработана Morten Nobel-Jørgensen. Шпаргалка фокусируется на языке, а не стандартной библиотеке.
Дополнения для C++11 основаны на [ISOCPP.org C++11 Cheatsheet](https://isocpp.org/blog/2012/12/c11-a-cheat-sheet-alex-sinyakov).

Цель - дать сжатый обзор основ современного C++.

Оригинал документа на английском размещён на [github.com/mortennobel/cpp-cheatsheet](https://github.com/mortennobel/cpp-cheatsheet).
...
## ASSERT.H, CASSERT (Отладочный механизм) 
~~~~
#include <cassert>          // Включаем cassert
assert(cond);               // Если условие cond не выполняется, распечатать сообщение и аварийно завершить программу
                            // В Release-конфигурациях объявлен макрос NDEBUG, который превращает assert в пустой макрос
                            //  (пустой макрос ничего не выполняет и не вычисляет выражение cond)
~~~~ 
